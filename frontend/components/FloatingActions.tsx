...
/**
 * WhatsApp, and only WhatsApp. It is the one action that must be reachable at
 * any scroll position on any page.
 *
 * Book Now is deliberately NOT here: on desktop it sits in the header, and on
 * phones it lives in the menu. Two floating buttons crowded a 360px screen.
 */
export default function FloatingActions() {
  ...
  return (
    <a
      ...
      className="group btn-outline fixed bottom-5 right-5 z-50 inline-flex items-center gap-2.5 rounded-full bg-surface/90 px-4 py-3 font-body text-xs tracking-wide shadow-lift backdrop-blur-xl focus:outline-none focus-visible:ring-1 focus-visible:ring-accent sm:bottom-6 sm:right-6 sm:px-5"
      style={{ marginBottom: "env(safe-area-inset-bottom)" }}
    >
      <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" aria-hidden="true" className="text-accent transition-colors duration-300 group-hover:text-accent-fg">
