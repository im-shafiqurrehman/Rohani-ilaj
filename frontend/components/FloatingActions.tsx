...
/**
 * Persistent actions, bottom right, on every page and at every scroll position.
 *
 * Book sits above WhatsApp because it is the primary action and the higher
 * position is the easier thumb reach. On phones WhatsApp collapses to an icon
 * so the stack does not crowd the screen edge; on wider screens it carries its
 * label.
 */
export default function FloatingActions() {
  ...
  return (
    ...
    >
      <Link
        href="/booking"
        className="glow-button inline-flex items-center gap-2 rounded-full bg-accent px-5 py-3 font-body text-xs tracking-wide text-accent-fg shadow-lift sm:text-sm"
      >
      ...
      <a
        ...
        className="btn-outline inline-flex items-center gap-2.5 rounded-full bg-surface/90 p-3 font-body text-xs tracking-wide shadow-lift backdrop-blur-xl focus:outline-none focus-visible:ring-1 focus-visible:ring-accent sm:px-5 sm:py-3"
      >
        <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" aria-hidden="true" className="text-accent">
